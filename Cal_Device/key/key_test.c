 ...
 * Description: Key control test program
 *            : with XHYPER270-TKU
 * =================================================
 */
#include <stdio.h>
...
{   
    ...
    if (tmo){ 
        if (tmo < 0)
            ...
            tmo *= 1000000;
        while (tmo > 0){
            usleep(400000);/* delay for showing strings perfectly*/
            ioctl(keyFd, FIONREAD, &b); /* make the value of keyboard*/
            if (b) return(b);
            tmo -= 10000;
        }
        return(-1);
...
int main()
{
	...
	while(1){
		c = key_get(10);/* get the value of key*/
		switch (c){/* print strings to minicom*/
